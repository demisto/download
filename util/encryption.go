...
func Decrypt(ciphertext string, key []byte) (string, error) {
	...
	// If decryption is wrong and padding is weird
	if padding < 0 || padding > ciph.BlockSize() {
		return "", fmt.Errorf("Encryption key is wrong - %v", key)
	}
	...
	if !hmac.Equal(plainbytes[len(cleanbytes):len(cleanbytes)+mac.Size()], chksum) {
		return "", fmt.Errorf("Could not validate cleartext - %v", cleanbytes)
	}
